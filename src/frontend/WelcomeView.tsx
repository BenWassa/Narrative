...
interface WelcomeViewProps {
  isOpen: boolean;
  standalone?: boolean; // when true, render as a standalone full-page view (not modal overlay)
  onClose: () => void;
...
export default function WelcomeView({
  ...
  // Modal rendering (fallback behavior)
  return (
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          {recentProjects.map(p => (
            ...
            >
              ...
              >
                {p.coverUrl ? (
                  <img src={p.coverUrl} alt={p.projectName} className="w-full h-36 object-cover" />
                ) : (
                  <div className="w-full h-36 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-sm text-gray-400">
                    {p.projectName}
                  </div>
